...
db_test!(test_merge_events_to_db_basic, (|pool: PgPool| async move {
    let registry = TaskSetRegistry::new();
    ...
    let client = pool.get().await.unwrap();
    let cursor_rows = client.query("SELECT last_event_id FROM merge_cursor", &[]).await.unwrap();
    assert_eq!(cursor_rows.len(), 1);
    
    println!("✅ Basic merge_events_to_db test passed!");
}));

db_test!(test_merge_events_to_db_comprehensive, (|pool: PgPool| async move {
    let registry = TaskSetRegistry::new();
    let domain = "test_domain";
