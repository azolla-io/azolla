...
2. **Trait-Based Tasks** (manual `Task` implementation):
   ...
   - Use `task.execute_json(json_args)` to test JSON parsing (simulates worker)
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match task_instance.execute_json(args).await {
        Ok(result) => {
...
**Note**: All examples are now available from the repo root and include proc macro functionality.
