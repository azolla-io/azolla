...
fn test_retry_policy_integration() {
    ...
    assert_eq!(policy.retry.include_errors, vec!["NetworkError", "TimeoutError", "ServiceUnavailable"]);
    assert_eq!(policy.retry.exclude_errors, vec!["AuthenticationError", "ValidationError"]);
...
fn test_type_conversion_error_handling() {
    ...
    for (value, expected_type) in test_cases {
        let result = match expected_type {
            "i32" => <i32 as FromJsonValue>::try_from(value.clone()).map(|_| ()).map_err(|e| e.to_string()),
            "String" => <String as FromJsonValue>::try_from(value.clone()).map(|_| ()).map_err(|e| e.to_string()),
            "bool" => <bool as FromJsonValue>::try_from(value.clone()).map(|_| ()).map_err(|e| e.to_string()),
            "Array" => <Vec<i32> as FromJsonValue>::try_from(value.clone()).map(|_| ()).map_err(|e| e.to_string()),
            "Vec<i32>" => <Vec<i32> as FromJsonValue>::try_from(value.clone()).map(|_| ()).map_err(|e| e.to_string()),
            _ => continue,
        };
        
        assert!(result.is_err(), "Expected error for {expected_type} conversion of {value:?}");
