...
// Test types for argument parsing
#[derive(Debug, Deserialize, PartialEq)]
struct NoArgsStruct {
    // Empty struct for no-args testing
}
...
fn test_parse_args_single() {
    ...
    let parsed = MockTask::parse_args(args).unwrap();
    assert_eq!(parsed, 42);
}
...
fn test_parse_args_multiple() {
    ...
    let parsed = MultiArgMockTask::parse_args(args).unwrap();
...
fn test_parse_args_invalid_single() {
    ...
    let result = MockTask::parse_args(args);
    assert!(result.is_err());
...
fn test_parse_args_invalid_multiple() {
    ...
    let result = MultiArgMockTask::parse_args(args);
    assert!(result.is_err());
...
fn test_parse_args_empty() {
    ...
    let args = vec![];
    let result = NoArgMockTask::parse_args(args);
    assert!(result.is_ok());
