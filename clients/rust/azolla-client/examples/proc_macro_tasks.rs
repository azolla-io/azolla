// NOTE: This example demonstrates the proc macro functionality
// For a fully functional client, use the standalone azolla-client crate: 
// https://crates.io/crates/azolla-client (when published)
// Or see clients/rust/azolla-client/

use azolla::client::{azolla_task, Client, TaskError, Worker, Task, TaskResult, FromJsonValue};
...
#[azolla_task]
async fn process_data(input: String, format: Option<String>, validate: bool) -> Result<Value, TaskError> {
    let format = format.unwrap_or_else(|| "json".to_string());
    
    if validate && input.len() < 5 {
        return Err(TaskError::execution_failed("Input too short for validation"));
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let worker = Worker::builder()
        ...
        .domain("proc-macro-examples")
        .shepherd_group("rust-proc-macro-workers")
    ...
    // In a real application, you would run the worker:
    // worker.run().await?;
