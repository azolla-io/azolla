...
mod tests {
    ...
    fn test_retry_policy_defaults() {
        ...
        assert!(policy.retry.include_errors.contains(&"ValueError".to_string()));
        assert!(policy.retry.exclude_errors.is_empty());
        
        // Check wait strategy is exponential jitter by default
        match policy.wait {
            WaitStrategy::ExponentialJitter { initial_delay, multiplier, max_delay } => {
    ...
    fn test_retry_policy_builder() {
        ...
        assert_eq!(policy.retry.include_errors, vec!["NetworkError", "TimeoutError"]);
    ...
    fn test_retry_policy_serialization() {
        ...
        assert_eq!(policy.retry.include_errors, deserialized.retry.include_errors);
        assert_eq!(policy.retry.exclude_errors, deserialized.retry.exclude_errors);
    }
    ...
    fn test_wait_strategy_serialization() {
        let strategies = vec![
            WaitStrategy::Fixed { delay: Duration::from_secs(5) },
        ...
        for strategy in strategies {
            ...
            match (&strategy, &deserialized) {
                ...
                (
                    WaitStrategy::Exponential { initial_delay: id1, multiplier: m1, max_delay: md1 },
                    WaitStrategy::Exponential { initial_delay: id2, multiplier: m2, max_delay: md2 }
                ) => {
                    assert_eq!(id1, id2);
                    assert_eq!(m1, m2);
                    assert_eq!(md1, md2);
                ...
                (
                    WaitStrategy::ExponentialJitter { initial_delay: id1, multiplier: m1, max_delay: md1 },
                    WaitStrategy::ExponentialJitter { initial_delay: id2, multiplier: m2, max_delay: md2 }
                ) => {
                    assert_eq!(id1, id2);
                    assert_eq!(m1, m2);
                    assert_eq!(md1, md2);
    ...
    fn test_builder_method_chaining() {
        ...
        assert_eq!(policy.retry.include_errors, vec!["IOError", "ConnectionError"]);
    ...
    fn test_edge_cases() {
        ...
        let zero_attempts = RetryPolicy::builder()
            .max_attempts(0)
            .build();
    ...
    fn test_wait_strategy_debug() {
        let fixed = WaitStrategy::Fixed { delay: Duration::from_secs(1) };
    ...
    fn test_retry_condition_patterns() {
        ...
        assert!(condition.include_errors.contains(&"NetworkError".to_string()));
        assert!(condition.exclude_errors.contains(&"AuthenticationError".to_string()));
    }
