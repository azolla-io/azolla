...
impl DispatchService {
    pub fn new(shepherd_manager: Arc<ShepherdManager>, task_registry: Arc<TaskSetRegistry>) -> Self {
        Self {
            shepherd_manager,
            task_registry,
        }
...
impl Dispatch for DispatchService {
    ...
    async fn stream(
        ...
        tokio::spawn(async move {
            if let Err(e) = handle_shepherd_connection(client_stream, tx.clone(), shepherd_manager, task_registry).await {
                error!("Shepherd connection error: {}", e);
                let _ = tx.send(Err(tonic::Status::internal("Connection failed"))).await;
            }
        });
...
async fn handle_hello_message(
    ...
    info!("Shepherd {} registering with max_concurrency={}", uuid, hello.max_concurrency);
    
    shepherd_manager.register_shepherd(uuid, hello.max_concurrency).await?;
...
async fn handle_status_message(
    ...
    if let Some(uuid) = shepherd_uuid {
        shepherd_manager.update_shepherd_status(*uuid, status.current_load, status.available_capacity);
        debug!("Updated status for shepherd {}: load={}, available={}", 
...
async fn handle_task_result_message(
    ...
    if let Some(uuid) = shepherd_uuid {
        ...
        info!("Received result for task {} from shepherd {}", task_id, uuid);
