...
impl StreamHandler {
    ...
    async fn connect_and_handle(&mut self) -> Result<()> {
        info!("Connecting to orchestrator at {}", self.config.orchestrator_endpoint);
        
        let channel = Channel::from_shared(self.config.orchestrator_endpoint.clone())?.connect().await?;
        ...
        tx.send(hello_msg).await.map_err(|e| anyhow::anyhow!("Failed to send hello: {}", e))?;
    ...
    async fn handle_server_message(&mut self, server_msg: ServerMsg) -> Result<()> {
        match server_msg.kind {
            Some(server_msg::Kind::Task(task)) => {
                ...
                self.event_sender.send(StreamEvent::TaskReceived(incoming_task))
    ...
    async fn send_task_result(&self, tx: &mpsc::Sender<ClientMsg>, task_result: TaskResultMessage) -> Result<()> {
