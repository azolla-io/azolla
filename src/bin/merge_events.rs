...
#[command(about = "Utility to merge events from events table to task_instance and task_attempts tables")]
struct Args {
    /// Show verbose output
    #[arg(short, long)]
    verbose: bool,
...
async fn get_merge_stats(pool: &PgPool) -> Result<MergeStats> {
    ...
    let last_event_id = cursor_row
        .map(|row| row.get::<_, i64>(0))
        .unwrap_or(0);
    
    // Get total events count
    let total_events_row = client
    ...
    let unprocessed_events_row = client
        .query_one("SELECT COUNT(*) FROM events WHERE event_id > $1", &[&last_event_id])
        .await?;
...
async fn main() -> Result<()> {
    ...
    match registry.merge_events_to_db(&pool).await {
        Ok(()) => {
            ...
            if args.verbose {
                println!("  Final last processed event ID: {}", final_stats.last_event_id);
                println!("  Remaining unprocessed events: {}", final_stats.unprocessed_events);
            }
        }
        Err(e) => {
            let duration = start_time.elapsed();
            eprintln!("❌ Event merge failed after {:.2}s: {}", duration.as_secs_f64(), e);
            std::process::exit(1);
        }
    }
